...
def get_total_users(db: Session = Depends(get_db), _=Depends(get_current_user)):
    count = db.query(User).count()
    return {"count": count}

@router.get("/stats/doctors")
def get_total_doctors(db: Session = Depends(get_db), _=Depends(get_current_user)):
    count = db.query(User).filter(User.role == RoleEnum.DOCTOR).count()
    return {"count": count}

@router.get("/stats/labtechs")
def get_total_labtechs(db: Session = Depends(get_db), _=Depends(get_current_user)):
    count = db.query(User).filter(User.role == RoleEnum.LAB_TECHNICIAN).count()
    return {"count": count}
...
def get_all_test_requests(
    ...
    return db.query(TestRequest).all()
...
def get_all_results_for_admin(
    ...
    # Optional: ensure only admins access this
...
def get_user_detail(user_id: int, db: Session = Depends(get_db)):
    ...
    if user.role.lower() == "doctor":
        test_requests = db.query(TestRequest).filter(TestRequest.doctor_id == user.id).all()
        ...
        test_results = db.query(TestResult).filter(TestResult.technician_id == user.id).all()
        return UserDetailOut(
            id=user.id,
            full_name=user.full_name,
            email=user.email,
        ...
        # For patients or other roles, you can customize accordingly
        return UserDetailOut(
            id=user.id,
            full_name=user.full_name,
            email=user.email,
